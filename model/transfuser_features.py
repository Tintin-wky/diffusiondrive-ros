...
from .nuplan.abstract_map import AbstractMap, SemanticMapLayer, MapObject
from .nuplan.oriented_box import OrientedBox
from .nuplan.state import StateSE2
from .nuplan.tracked_objects_types import TrackedObjectType

from .transfuser_config import TransfuserConfig
from .dataclasses import AgentInput, Scene, Annotations
from .local_navsim import AbstractFeatureBuilder, AbstractTargetBuilder, BoundingBoxIndex, LidarIndex
    ...
    def _get_lidar_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        def splat_points(point_cloud):
            ...
            xbins = np.linspace(
                ...
                int((self._config.lidar_max_x - self._config.lidar_min_x) * int(self._config.pixels_per_meter) + 1),
            )
            ybins = np.linspace(
                self._config.lidar_min_y,
                self._config.lidar_max_y,
                int((self._config.lidar_max_y - self._config.lidar_min_y) * int(self._config.pixels_per_meter) + 1),
            )
