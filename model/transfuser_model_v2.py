...
from .local_navsim import StateSE2Index
...
class V2TransfuserModel(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        num_bev_tokens = config.lidar_vert_anchors * config.lidar_horz_anchors
        num_embeddings = num_bev_tokens + 1 if config.include_status_in_keyval else num_bev_tokens
        self._keyval_embedding = nn.Embedding(num_embeddings, config.tf_d_model)  # BEV feature grid + optionally status
        self._query_embedding = nn.Embedding(sum(self._query_splits), config.tf_d_model)

        # usually, the BEV features are variable in size.
        self._bev_downscale = nn.Conv2d(512, config.tf_d_model, kernel_size=1)
        if config.include_status_in_keyval:
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None) -> Dict[str, torch.Tensor]:
        ...
        batch_size = status_feature.shape[0] # 64
        ...
        if self._config.include_status_in_keyval:
        ...
        if self._config.include_status_in_keyval:
            ...
            bev_part = keyval
        concat_cross_bev = bev_part.permute(0,2,1).contiguous().view(batch_size, -1, concat_cross_bev_shape[0], concat_cross_bev_shape[1])
...
class AgentHead(nn.Module):
    ...
    def forward(self, agent_queries) -> Dict[str, torch.Tensor]:
        ...
        agent_states[..., :2] = agent_states[..., :2].tanh() * 32
        agent_states[..., BoundingBox2DIndex._HEADING] = agent_states[..., BoundingBox2DIndex._HEADING].tanh() * np.pi 
...
class CustomTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, 
        ...
        poses_reg[..., StateSE2Index._HEADING] = poses_reg[..., StateSE2Index._HEADING].tanh() * np.pi
